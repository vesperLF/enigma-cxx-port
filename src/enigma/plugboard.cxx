...
plugboard::plugboard(const std::set<std::pair<char, char>>& connections) {
    wiring.resize(26);
    for (int i = 0; i < 26; i++) {
        wiring[i] = i;
    }
...
std::set<int> plugboard::get_unplugged_characters(const std::set<std::pair<char, char>>& connections) {
